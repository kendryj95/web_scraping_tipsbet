...
const getRawData = (URL, idMatch = null) => {
    const data = idMatch ? {
        action: "getMatchBetInformation",
        matchid: idMatch
    } : null;
    const jsonData = data ? {
        method: 'post',
    ...
    const fetchData = jsonData ? fetch(URL, jsonData) : fetch(URL);
...
const urlOdds = `http://www.statarea.com/actions/controller`;
        ...
        for (let i = 0; i < competitions.length; i++) {
            ...
            if (tip == "1" || tip == "1X" || tip == "2" || tip == "X2" || tip == "12") {
                ...
                if (tipTeam1 >= 59 || tipTeam2 >= 59) {
                    const idMatch = parseInt(competitions[i].attribs.id);
                    const rawDataOdds = await getRawData(urlOdds, idMatch);
                    const $ = cheerio.load(rawDataOdds);
                    console.log(idMatch);
                    console.log(rawDataOdds);
                    console.log($('div.halfcontainer')[0]);
                    break;
                    n++;
